...
test('项目扫描解析状态、递归文件数和 README 摘要', async (t) => {
  ...
  const completedPath = path.join(root, '示例项目-已完成');
  ...
  const noStatusPath = path.join(root, '无状态目录');
  ...
  const completed = projects.find((project) => project.dirName === '示例项目-已完成');
  assert.strictEqual(completed.name, '示例项目');
  ...
  const unmarked = projects.find((project) => project.dirName === '无状态目录');
  assert.strictEqual(unmarked.name, '无状态目录');
